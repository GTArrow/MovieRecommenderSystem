...
export async function enrichPreferences(preferences: { liked_movie_id: number; createdAt: Date }[]) {
  return Promise.all(
    preferences.map(async (pref) => {
      let movie = null;
      try {
        movie = await fetchMovieById(pref.liked_movie_id);
      } catch (error) {
...
export async function enrichPreferredGenres(preferredGenres: { genre_id: number; createdAt: Date }[]) {
